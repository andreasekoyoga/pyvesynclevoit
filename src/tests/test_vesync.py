...
class TestVesync(unittest.TestCase):
    ...
    def test_login(self):
        ...
        with patch('pyvesynclevoit.helpers.Helpers.call_api') as mocked_post:
            d = {
                'result': {
                    'accountID': '12346536',
                    'userType': '1',
...
class TestApiFunc:
    ...
    @patch('pyvesynclevoit.helpers.requests.get', autospec=True)
    ...
    @patch('pyvesynclevoit.helpers.requests.post', autospec=True)
    ...
    @patch('pyvesynclevoit.helpers.requests.put', autospec=True)
