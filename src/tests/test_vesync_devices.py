...
import logging
import pyvesynclevoit
import time
import copy
from itertools import chain
from pyvesynclevoit import (
    VeSyncAir131,
    VeSyncBulbESL100,
    VeSyncOutdoorPlug,
    VeSyncOutlet10A,
...
class TestDeviceList(object):
    ...
    def api_mock(self, caplog):
        ...
        self.mock_api_call = patch('pyvesynclevoit.helpers.Helpers.call_api')
        self.mock_api = self.mock_api_call.start()
        self.mock_api.create_autospec()
        self.mock_api.return_value.ok = True
        self.vesync_obj = pyvesynclevoit.vesync.VeSync('sam@mail.com', 'pass')
    ...
    @patch('pyvesynclevoit.vesync.VeSyncOutlet7A')
    @patch('pyvesynclevoit.vesync.VeSyncOutlet15A')
    @patch('pyvesynclevoit.vesync.VeSyncOutlet10A')
    @patch('pyvesynclevoit.vesync.VeSyncWallSwitch')
    @patch('pyvesynclevoit.vesync.VeSyncDimmerSwitch')
    @patch('pyvesynclevoit.vesync.VeSyncAir131')
    ...
    def test_unknown_device(self, caplog, api_mock):
        ...
        pyvesynclevoit.vesync._device_builder('unknown_device', unknown_dev, self.vesync_obj)
    ...
    @patch('pyvesynclevoit.vesync.VeSyncOutlet7A', autospec=True)
    def test_remove_device(self, outlet_patch, caplog, api_mock):
        ...
        device_exists = pyvesynclevoit.vesync.VeSync.remove_old_devices(
        ...
        device_exists = pyvesynclevoit.vesync.VeSync.remove_dev_test(outlet_test, new_list)
    ...
    @patch('pyvesynclevoit.vesync.VeSyncOutlet7A', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncOutlet15A', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncOutlet10A', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncOutdoorPlug', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncBulbESL100', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncWallSwitch', autospec=True)
    @patch('pyvesynclevoit.vesync.VeSyncAir131', autospec=True)
